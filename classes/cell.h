...
class Cell
{
  public:
    ...
    double u;                 //second dimensional state variable
    double u_temp;            //second dimensional state variable
    ...
    inline int calc(OutputLogService &LogService, int properDynamics, bool logSpiking)
    {
        ...
        else if ((voltage >= (al + u_temp)) || last_last_voltage > 0)
        {
            ...
            /*num_fired = num_fired + 1;*/ //TODO we can not use this now, it is global variable
        }
        else
        {
        ...
        //if(voltage >= -0.94 && last_voltage < -1.0){
        //printf("something weird is up voltage:%lf\n",voltage);
        //}
        //if(voltage <= -0.93 && voltage >= -0.94 && last_voltage < -0.95 ){
        //printf("something weird is up\n");
        //}

        input = input * 0.985; //0.99;//magic TEST VALUE!!!! //TODO why do we need this
        //if(i == 0){//Fix this

        //printf("input decay:%lf \n",input);
        //}
