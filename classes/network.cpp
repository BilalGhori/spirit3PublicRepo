...
//TODO rewrite this to Z,Y,X order
...
//TODO possible speedup - traverse via cached list of all connections instead of cells.
void Network::compute_STDP(Cell &cell)
{
  time_type t = Time.ctime; //current spike time

  //TODO this is not entirely correct - if spike happens at the same time in two connected cells we shouldn't use spike from spike_log from aready updated cell
  // now we go here only through cells which fired, not all
...
void Network::apply_STDP_traces(double dopamine)
{
  ...
  for (int i = 0; i < cells; i++)
  {
    ...
    for (; cit != cend; cit++)
    {
      (*cit)->clean_trace(Time.ctime); //TODO check optimization - we don't need the window check in add_pre_post_stdp_trace if we do it here; since this is central loop we maybe do not want clean to be here
      ...
      //TODO look whether we can sum the traces before applying them (in order to save time)
      tit = (*cit)->trace.begin();
      for (; tit != tend; tit++)
      {
  ...
  if(enableInputBalencingOverride == 1 && EnableInputBalencing)
    input_balancing();
  output_balancing(); //TODO does not work currently correctly
} //apply_STDP_traces
...
void Network::init_balancing()
{
  ...
  for (int i = 0; i < cells; i++)
  {
    ...
    for (sum = 0; cit != cend; cit++)
      if ((*cit)->strength > 0)
        sum += (*cit)->strength;
    tmp.output_target = sum; //tmp.output_target=tmp.output_sum; //TODO check this
...
//biol. could correspond to running out of inner cell resources, does not work correctly see todo
void Network::output_balancing()
{
  for (int i = 0; i < cells; i++)
  {
    ...
    //TODO to enable output balancing uncomment line below
    //cit=tmp.out_connections.begin();
    for (; cit != cend; cit++)
      if ((*cit)->strength > 0 && (*cit)->freeze == false)
      {
